...
It is a service: a FastAPI application under `src/test_repo/app.py`, served by uvicorn.
## Tasks
...
- The service exposes a health endpoint at `/health` and its OpenAPI docs at `/docs`.
- It runs locally with `uvicorn test_repo.app:app --reload`; it needs no external service to start.
